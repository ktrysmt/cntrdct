# OSF Preregistration: cntrdct β-phase precision and recall study (revision 2)
...
## What changed since 2026-05-05

The previous revision committed to five Layer 1 detectors with three
of them (`unreachable-after-terminator`, `comment-code`, `arg-swap`)
accepting Python source. Between 2026-05-05 and the date of this
revision the project landed the final M-3 sub-task: Python extension
of `clone-drift`. This revision restates the full preregistration so
that the most recent dated document continues to commit the entire
detector and citation surface that will be measured at β release
time. The substantive changes are:

- The detector surface still names the same five detectors. Four of
  them (`unreachable-after-terminator`, `comment-code`, `arg-swap`,
  and now `clone-drift` added in this revision) accept Python source
  in addition to Rust. `config-interaction` remains Rust-only by
  design (Pattern B per `docs/spec/multilang-v0.md`). The Rust-side
  hypotheses are unchanged.
- `clone-drift` Python ships with `LanguageCitationStatus::Confirmed`
  via Assi, Hassan, Zou (ACM TOSEM 2025) — an independent
  peer-reviewed application of NiCad and SourcererCC to nine
  open-source Python deep-learning frameworks, satisfying clause (b)
  of `docs/spec/citations-policy.md` for `cordy-roy-icpc-2008` and
  the inconsistent-change framing for `bettenburg-msr-2009` and
  `krinke-icsm-2007`. See
  `docs/surveys/clone-drift-python-2026-05.md`.
- The clone-drift detector adds a new tunable `MIN_FN_TOKENS = 22`
  that filters out trivially short utility functions whose drift
  signal is too noisy in practice. This brings the detector in line
  with industrial NiCad / SourcererCC pipelines and is documented in
  the updated `docs/spec/clone-drift-v0.md`. The change is
  multi-language and is exposed as `pub const` for tuning without
  API churn.
- The Layer 1 citation key set gains `assi-tosem-2025`.
- M-3 closes with this revision. M-4 (Python β corpus collection)
  remains the next preregistered work item; this revision's
  hypotheses remain scoped to the Rust corpus.
...
- Sensitivity of precision and recall to the seed corpus's
  ...
  `MIN_GROUP_SIZE`, `MIN_FN_TOKENS`, and the `comment-code`
  pattern set). Tuning these in response to the preregistered
  figures is a separate, post-hoc activity.
...
identifier is `osf-prereg-2026-05-06`. The previous identifiers
`osf-prereg-2026-05-03` and `osf-prereg-2026-05-05` continue to
refer to the superseded revisions. Detectors that wish to reference
this preregistration set `preregistration_id` accordingly.
