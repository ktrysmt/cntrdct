...
impl Detector for ArgSwap {
    ...
    fn detect(&self, ctx: &DetectContext) -> Result<Vec<Finding>, DetectorError> {
        ...
        // R-3.d: Go reuses the same name-correlation pipeline — IR
        // definition extraction + a raw-tree call walk (Pattern B).
        // Unconfirmed until the R-3.f survey grounds a Go citation.
        findings.extend(run_pipeline(
            ctx,
            Language::Go,
            extract_go_fn_defs,
            extract_go_call_sites,
            LanguageCitationStatus::Unconfirmed,
            &["li-zhou-fse-2005", "rice-icse-2017"],
        ));
...
// ---------- Go (R-3.d) ----------
//
// Go definitions come from IR (function + method declarations; the
// receiver lives in a separate field so `ir_fn_to_def` sees only the real
// args). Call-site enumeration is a raw-tree walk (Pattern B) so calls
// nested in `IrExpr::Other` shapes — binary operands, closures, `defer` /
// `go` statements — stay visible, matching the arg-swap recall discipline.

fn extract_go_fn_defs(file: &IrFile) -> Option<Vec<(String, FnDef)>> {
    if file.parse_recovered {
        return None;
    }
    Some(file.fns.iter().filter_map(ir_fn_to_def).collect())
...
fn extract_go_call_sites(file: &IrFile) -> Option<Vec<CallSite>> {
    if file.parse_recovered {
        return None;
    }
    let tree = file.raw_tree();
    let mut calls = Vec::new();
    walk_go_for_calls(tree.root_node(), file, &mut calls);
    Some(calls)
}

fn walk_go_for_calls(node: tree_sitter::Node, file: &IrFile, out: &mut Vec<CallSite>) {
    if node.kind() == "call_expression" {
        if let Some(call) = parse_go_call(node, file) {
            out.push(call);
        }
    }
    let mut cursor = node.walk();
    for child in node.children(&mut cursor) {
        walk_go_for_calls(child, file, out);
    }
}

/// A Go call is in scope when its function operand is a bare identifier
/// (`foo(...)`) or a single-receiver method call `recv.Method(...)` whose
/// receiver is itself a bare identifier (the receiver is dropped and the
/// field name becomes the callee, mirroring how methods register as
/// definitions). Deeper selectors (`a.b.c(...)`) are out of v0 scope.
/// Every argument must be a bare `identifier`.
fn parse_go_call(node: tree_sitter::Node, file: &IrFile) -> Option<CallSite> {
    let function = node.child_by_field_name("function")?;
    let callee = match function.kind() {
        "identifier" => node_text(function, &file.source),
        "selector_expression" => go_single_receiver_method(function, &file.source)?,
        _ => return None,
    };
    ...
    for child in arguments.children(&mut cursor) {
        ...
        if child.kind() == "identifier" {
            ...
            // Literals, selectors, nested calls, composite literals —
            // anything other than a bare identifier disqualifies the call
            // from v0 swap analysis.
            return None;
        }
    }
...
/// Extract the method name from a `recv.Method` selector whose receiver is
/// a bare identifier, or `None` for any deeper / non-identifier receiver
/// (keeps the call shape to a single-segment method, matching the
/// TypeScript `this.<name>` restriction without a `this` keyword in Go).
fn go_single_receiver_method(selector: tree_sitter::Node, source: &str) -> Option<String> {
