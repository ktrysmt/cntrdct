//! Integration tests for `cntrdct-research fetch`.
//!
...
fn provenance_json_records_rank_source_when_metadata_present() {
    ...
    std::fs::create_dir_all(&out).unwrap();
    let list = tmp.path().join("crates.txt");
    // Synthesise the crate list as if `cntrdct-research rank` had emitted it,
    // including the comment-line provenance pin.
    std::fs::write(
        &list,
        "# generated by `cntrdct-research rank`\n\
...
fn run_rank_writes_sidecar_licenses_tsv_alongside_output() {
    ...
    // `cntrdct-research rank` is the only chance to capture licenses up-front from
