...
use cntrdct_corpus_fetch::{
    append_row, fetch_one, read_manifest_names, sha256_hex, ExtractOptions, FetchError,
    FetchOutcome, HttpClient, SparseIndexClient, TarballClient, DEFAULT_LICENSE_ALLOWLIST,
};
...
/// one-shot semantics; the orphan-recovery scenario re-invokes the
/// orchestrator over the same crate, so this mock must be idempotent on
/// repeated lookups.
